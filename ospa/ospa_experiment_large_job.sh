...
mkdir -p experiments_large
mkdir -p experiments_large/vanilla
mkdir -p experiments_large/ospa_init
mkdir -p experiments_large/ospa_regularize
mkdir -p experiments_large/ospa_strict
mkdir -p experiments_large/analysis
mkdir -p experiments_large/logs

# Log file for tracking experiment progress
MAIN_LOG="experiments_large/logs/experiment_progress.log"
...
run_experiment() {
    ...
    local exp_log="experiments_large/logs/${experiment_name}.log"
...
run_experiment "vanilla_transformer" "vanilla" "init" "0.0" "experiments_large/vanilla" ""

# Train OSPA with initialization only 
run_experiment "ospa_init" "ospa" "init" "0.0" "experiments_large/ospa_init" ""

# Train OSPA with regularization (medium strength only to save time)
run_experiment "ospa_regularize_medium" "ospa" "regularize" "0.001" "experiments_large/ospa_regularize" ""

# Train OSPA with strict orthogonality
run_experiment "ospa_strict" "ospa" "strict" "0.0" "experiments_large/ospa_strict" ""
...
analyze_model "experiments_large/vanilla/vanilla_transformer.pt" "experiments_large/analysis/vanilla" "vanilla"
analyze_model "experiments_large/ospa_init/ospa_init.pt" "experiments_large/analysis/ospa_init" "ospa_init"
analyze_model "experiments_large/ospa_regularize/ospa_regularize_medium.pt" "experiments_large/analysis/ospa_regularize" "ospa_regularize"
analyze_model "experiments_large/ospa_strict/ospa_strict.pt" "experiments_large/analysis/ospa_strict" "ospa_strict"
...
available_models=()
for model_path in "experiments_large/vanilla/vanilla_transformer.pt" "experiments_large/ospa_init/ospa_init.pt" "experiments_large/ospa_regularize/ospa_regularize_medium.pt" "experiments_large/ospa_strict/ospa_strict.pt"; do
    if [ -f "$model_path" ]; then
...
if [ ${#available_models[@]} -ge 2 ]; then
    ...
    python analyze_head_diversity.py\
        ...
        --output_dir experiments_large/analysis/comparison \
        --d_model 512 --nhead 8 --nlayers 6\
        --device cpu
