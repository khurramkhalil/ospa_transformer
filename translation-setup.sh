...
echo "Setting up OSPA experiments on IWSLT machine translation dataset..."

# First, download the IWSLT dataset
echo "Downloading IWSLT dataset..."
mkdir -p data/iwslt
cd data/iwslt

# Create a simple Python script to download the dataset
cat > download_iwslt.py << EOF
from datasets import load_dataset
import os

# Download IWSLT dataset (German-English)
dataset = load_dataset("iwslt2017", "iwslt2017-de-en")

# Save to disk
os.makedirs("de-en", exist_ok=True)
dataset.save_to_disk("de-en")

print("IWSLT dataset saved to disk")
EOF

python download_iwslt.py
cd ../../

# Create a translation training script
echo "Creating translation training script..."
cat > src/scripts/train_translation.py << 'EOF'
#!/usr/bin/env python3
"""
...
# Create a script to run translation experiments
echo "Creating run script for translation experiments..."
cat > run_iwslt_experiments.sh << 'EOF'
#!/bin/bash
...
chmod +x run_iwslt_experiments.sh

echo "IWSLT translation setup complete!"
echo "Run ./run_iwslt_experiments.sh to start experiments"